...
Tensor::Tensor(int n, int c, int h, int w, DataType dtype, std::shared_ptr<MixMemory> data,
               int device_id) {
    this->dtype_ = dtype;
...
Tensor::Tensor(const std::vector<int>& dims, DataType dtype, std::shared_ptr<MixMemory> data,
               int device_id) {
    this->dtype_ = dtype;
    this->device_id_ = get_device(device_id);
    descriptor_string_[0] = 0;
...
// 默认构造函数3
Tensor::Tensor(int ndims, const int* dims, DataType dtype, std::shared_ptr<MixMemory> data,
               int device_id) {
    this->dtype_ = dtype;
    this->device_id_ = get_device(device_id);
    descriptor_string_[0] = 0;
...
std::shared_ptr<Tensor> Tensor::clone() const {
    ...
    if(head_ == DataHead::Host) {
        ...
        AutoDevice auto_device_exchange(device());
...
static inline void memset_any_type(_T* ptr, size_t count, _T value) {
    for(size_t i = 0; i < count; ++i) *ptr++ = value;
}
...
Tensor& Tensor::to_gpu(bool copy) {
    if(head_ == DataHead::Device) return *this;
...
Tensor& Tensor::to_cpu(bool copy) {
    if(head_ == DataHead::Host) return *this;
...
Tensor& Tensor::to_float() {
    ...
    for(int i = 0; i < c; ++i) *dst++ = float16_to_float(*src++);
...
Tensor& Tensor::to_half() {
    if(type() == DataType::Float16) return *this;
    ...
    for(int i = 0; i < c; ++i) *dst++ = float_to_float16(*src++);
...
Tensor& Tensor::set_mat(int n, const cv::Mat& _image) {
    ...
    int width = shape_[3];
    int height = shape_[2];
    if(image.size() != cv::Size(width, height)) cv::resize(image, image, cv::Size(width, height));
    ...
    for(int i = 0; i < ms.size(); ++i) ms[i] = cv::Mat(height, width, CV_32F, cpu<float>(n, i));
...
Tensor& Tensor::set_norm_mat(int n, const cv::Mat& image, float mean[3], float std[3]) {
    ...
    cv::Mat ms[3];
    for(int c = 0; c < 3; ++c) ms[c] = cv::Mat(height, width, CV_32F, cpu<float>(n, c));

    split(inputframe, ms);
    Assert((void*)ms[0].data == (void*)cpu<float>(n));

    for(int c = 0; c < 3; ++c) ms[c] = (ms[c] - mean[c]) / std[c];
    return *this;
}
...
//
const char* Tensor::descriptor() const {  // 不能修改成员变量
...
// 据shape_中的信息, 返回当前shape字符串到 shape_string_ 中
