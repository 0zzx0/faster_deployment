 ...
 * @file tools.hpp
 * @author 0zzx0
 * @brief 一些工具函数 包括CUDA检查 输出文件保存读取等函数 全部在tools里面定义并直接实现
 * @version 0.1
...
///////////////////////TRT/////////////////////////////
#define TRT_STR(v) #v
#define TRT_VERSION_STRING(major, minor, patch, build) \
    TRT_STR(major) "." TRT_STR(minor) "." TRT_STR(patch) "." TRT_STR(build)
static const char* trt_version() {
    return TRT_VERSION_STRING(NV_TENSORRT_MAJOR, NV_TENSORRT_MINOR, NV_TENSORRT_PATCH,
                              NV_TENSORRT_BUILD);
}
...
static void __log_func(const char* file, int line, LogLevel level, const char* fmt, ...) {
    ...
    int n = snprintf(buffer, sizeof(buffer), "[%s][%s:%d]:", level_string(level), filename.c_str(),
                     line);
...
public:
    ...
    virtual ~AutoDevice() { checkCudaRuntime(cudaSetDevice(old_)); }
