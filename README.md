...
我们的目标肯定是吞吐量特别大，同时延迟超级小，在一定条件下，其实这两个是有互斥的。这也是表明程序性能(软件性能+硬件性能)两种方式，吞吐量代表并行能力，延迟代表串行效果。
...
需要注意，单幅图像的延迟，在我们这个层面是无法缩短的哦！因为一幅图像到推理器后端模型推理步骤的时候，必须经过`预处理->推理->后处理`三个步骤，这三个步骤的时间耗时在本阶段是无法缩小的(这是在之前优化网络结构和模型转换的时候考虑的)，本项目是希望在不增加单图延迟的基础上，尽量高的提升模型吞吐量，也就是尽量重合、去掉一些无用、重复、耗时的操作，对于机器人来说就是可以拥有更高频率的目标位置信息等。
