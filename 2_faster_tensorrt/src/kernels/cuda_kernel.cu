...
// yolox解码输出
static __global__ void yolox_decode_kernel(float* predict, int num_bboxes, int fm_area,
...
// yolox 后处理
void yolox_decode_kernel_invoker(float* predict, int num_bboxes, int fm_area, int num_classes,
                                 float confidence_threshold, float nms_threshold,
                                 float* invert_affine_matrix, float* parray, const float* prior_box,
...
// yolov8解码输出
static __global__ void yolov8_decode_kernel(float* predict, int num_bboxes, int fm_area,
                                            int num_classes, float confidence_threshold,
                                            float* invert_affine_matrix, float* parray,
                                            const float* prior_box, int max_objects) {
    int position = blockDim.x * blockIdx.x + threadIdx.x;
    ...
    float left = predict_cx - predict_w * 0.5f;
    float top = predict_cy - predict_h * 0.5f;
    float right = predict_cx + predict_w * 0.5f;
    float bottom = predict_cy + predict_h * 0.5f;
    affine_project(invert_affine_matrix, left, top, &left, &top);
    affine_project(invert_affine_matrix, right, bottom, &right, &bottom);
...
void yolov8_decode_kernel_invoker(float* predict, int num_bboxes, int fm_area, int num_classes,
    ...
    auto grid = grid_dims(num_bboxes);
    auto block = block_dims(num_bboxes);
    checkCudaKernel(yolov8_decode_kernel<<<grid, block, 0, stream>>>(
        predict, num_bboxes, fm_area, num_classes, confidence_threshold, invert_affine_matrix,
        parray, prior_box, max_objects));
...
// same to opencv
// reference:
// https://github.com/opencv/opencv/blob/24fcb7f8131f707717a9f1871b17d95e7cf519ee/modules/imgproc/src/resize.cpp
// reference:
// https://github.com/openppl-public/ppl.cv/blob/04ef4ca48262601b99f1bb918dcd005311f331da/src/ppl/cv/cuda/resize.cu

__global__ void resize_bilinear_and_normalize_kernel(uint8_t* src, int src_line_size, int src_width,
                                                     int src_height, float* dst, int dst_width,
                                                     int dst_height, float sx, float sy, Norm norm,
                                                     int edge) {
    int position = blockDim.x * blockIdx.x + threadIdx.x;
...
void resize_bilinear_and_normalize(uint8_t* src, int src_line_size, int src_width, int src_height,
                                   float* dst, int dst_width, int dst_height, const Norm& norm,
                                   cudaStream_t stream) {
...
static __global__ void warp_affine_bilinear_and_normalize_plane_kernel(
    ...
    int dst_height, uint8_t const_value_st, float* warp_affine_matrix_2_3, Norm norm, int edge) {
