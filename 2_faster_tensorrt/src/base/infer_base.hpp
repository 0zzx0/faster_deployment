...
/**
 * @brief 封装trt运行过程中的部分资源 方便一起创建和销毁
 *
 */
class EngineContext {
public:
    ...
    bool build_model(const void* pdata, size_t size) {
        ...
        engine_ =
            std::shared_ptr<ICudaEngine>(runtime_->deserializeCudaEngine(pdata, size, nullptr),
...
/**
 * @brief  异步线程安全的推理器(虚基类 子类至少重写preprocess work)
    通过异步线程启动，使得调用方允许任意线程调用把图像做输入，并通过future来获取异步结果
    模板类
 *
 * @tparam Input 输入
 * @tparam Output 输入
 * @tparam StartParam 参数
 * @tparam JobAdditional job参数
 */
...
public:
    ...
    virtual ~ThreadSafedAsyncInfer() { stop(); }
    ...
    bool startup(const StartParam& param) {
        ...
        start_param_ = param;
        worker_ =
            std::make_shared<std::thread>(&ThreadSafedAsyncInfer::worker, this, std::ref(pro));
...
/**
 * @brief 推理的虚基类 最终暴露给用户的接口，实际推理的类应该继承并实现本类中的方法
 *
 * @tparam Intput 输入
 * @tparam Output 输出
 */
